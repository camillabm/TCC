...
RAPA <- read_excel("FAZER/Autophagy states datasets/expression_matrix/raw_counts/Block/amb_km.xlsx")
write.csv(RAPA, "FAZER/Autophagy states datasets/expression_matrix/raw_counts/Block/amb_km.csv")
RAPA <- read.csv("FAZER/Autophagy states datasets/expression_matrix/raw_counts/Block/amb_km.csv")
...
gene_lengths_filtered <- gene_lengths_df %>%
  filter(SYMBOL %in% rownames(counts_matrix))#Change
gene_ids_valid <- gene_lengths_df$SYMBOL #Change
counts_matrix <- counts_matrix[rownames(counts_matrix) %in% gene_ids_valid, ]

counts_matrix <- counts_matrix %>%
  as.data.frame() %>%
  tibble::rownames_to_column(var = "Gene_ID")
counts_with_lengths <- left_join(counts_matrix, gene_lengths_filtered, by = c("Gene_ID" = "SYMBOL")) #Change
...
count_data <- counts_with_lengths[, !(names(counts_with_lengths) %in% c("Gene_ID", "entrez_id", "gene_length", "gene_length_kb", "ENSEMBL"))] #Change
...
write.csv(tpm_matrix, file = "FAZER/Autophagy states datasets/expression_matrix/tpm/Inhibition/amb_km.csv", row.names = TRUE)
